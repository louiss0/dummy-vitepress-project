...
const vueLanguageCore = require('@volar/vue-language-core');
const shared_esmBundler = require('./shared/vite-plugin-vue-nested-sfc.19673363.cjs');

function pascalCase(str) {
  return shared_esmBundler.capitalize(shared_esmBundler.camelize(str));
}
const plugin = (ctx) => {
  const ts = ctx.modules.typescript;
  const plugins = vueLanguageCore.getDefaultVueLanguagePlugins(ts, ctx.compilerOptions, {
  ...
  return {
    ...
    getEmbeddedFileNames(fileName, sfc) {
      ...
      for (const block of componentBlocks) {
        ...
        if (!vueFile) {
          vueFile = new vueLanguageCore.VueFile(id, snapshot, ts, plugins);
...
`
        ...
        const codeLength = vueLanguageCore.getLength(embeddedFile.content);
        if (sfc.scriptSetup) {
          vueLanguageCore.replace(
...
`
        ...
        const lengthShift = vueLanguageCore.getLength(embeddedFile.content) - codeLength;
